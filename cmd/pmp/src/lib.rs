...
fn pmpcmd(
    ...
    // currently only supports riscv
    if let Some(goblin::elf::header::EM_RISCV) = hubris.arch {
    } else {
        println!("`humility pmp` only supports riscv");
        return Ok(());
    }
    ...
    //Currently assumes at most pmpaddr0...pmpaddr15
    for offset in 0..16 {
        let pmpaddr = core.read_reg(
            hubris.register_from_id(base_addr + offset as u32).unwrap(),
        );
        // not all pmpaddrs will be implemented, so stop after the last valid csr.
